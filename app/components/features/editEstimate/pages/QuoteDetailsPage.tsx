...
export function QuoteDetailsPage() {
  ...
  const calculateSectionTotal = (items: LineItem[]) => {
    return items
      .reduce((total, item) => {
        return (
          total +
          (parseFloat(calculateLineTotal(item.quantity, item.price)) || 0)
        );
      }, 0)
      .toFixed(2);
  };
    ...
    setQuotes(
      quotes.map((quote) => {
        if (quote.id === quoteId) {
          return {
            ...quote,
            sections: quote.sections.filter(
  ...
  const renderItem =
    (quoteId: string, sectionId: string) =>
    ({ item, drag, isActive }: RenderItemParams<LineItem>) => {
      return (
        <ScaleDecorator>
          <TouchableOpacity
            onLongPress={drag}
        ...
        <View style={styles.quoteDetailsContent}>
            ...
            <View style={[styles.section, styles.profitMarginSection]}>
              ...
              <Slider
                ...
                onValueChange={(value: any) =>
                  handleUpdateQuote(quote.id, "profitMargin", value)
                }
          ...
          <View style={[styles.section, styles.notesSection]}>
            ...
            <TextInput
              ...
              value={quote.notes}
              onChangeText={(value) =>
                handleUpdateQuote(quote.id, "notes", value)
              }
